...
Node.js + Express + SQLite backend that powers every feature in the Roti & More app:
...
## Stack
- **Express** — HTTP API
- **better-sqlite3** — embedded SQL database (file `roti.db`, no separate server)
- **jsonwebtoken** — 30-day JWTs carrying `{ sub, mobile, role }`
- OTP delivery is pluggable: **dev mode** (default), **MSG91**, **Twilio Verify**, or **Firebase** (client-side)

## Setup
```bash
cd backend
npm install
cp .env.example .env      # set JWT_SECRET and ADMIN_MOBILES
npm run seed              # optional: demo products/customers/orders
...
For **Firebase Phone Auth**, do the OTP on the client with the Firebase SDK, then
send the resulting Firebase ID token to a `/auth/firebase` endpoint that verifies it
with the Admin SDK and issues your own JWT (same allowlist logic). Stub not included
to avoid the extra dependency — wire it if you choose Firebase.
...
|---|---|---|---|
| POST | `/auth/request-otp` | `{mobile}` | Sends OTP. Dev mode returns `{devCode}`. |
| POST | `/auth/verify-otp` | `{mobile, code}` | → `{token, role, profileComplete, user}` |
